...
class SimpleRouter < Trema::Controller
  ...
  #show its routing table
  MAX_NETMASK_LENGTH = 32
  def show_table(dpid)
  	table = @routing_table.show() 
  	logger.info "Destination\t\tNetmask\t\tNext_hop"
  	MAX_NETMASK_LENGTH.downto(0).each do |each_mask|
  		table_array = table[each_mask].to_a
  		table_array.each do |each|
  			logger.info "#{IPv4Address.new(each[0]).mask(each_mask)}\t\t#{each_mask}\t\t#{each[1]}"
  		end
  	end
  end
  
  #Add entry to the table
  def add_entry(dpid, destination, netmask_length, next_hop)
  	entry = Hash.new
  	entry[:netmask_length] = netmask_length
  	entry[:destination] = destination
  	entry[:next_hop] = next_hop
  	@routing_table.add(entry)
  	logger.info "Entry added [dest: #{destination}, mask: #{netmask_length}, next: #{next_hop}]"
  end
  
  #Delete entry from the table
  def delete_entry(dpid, destination)
  	destination_ip_address = IPv4Address.new(destination)
  	result = @routing_table.delete(destination_ip_address)
  	if result == true
 			logger.info "Entry for #{destination} has been deleted"
 		else
 			logger.info "Entry for #{destination} not found"
 		end
  end
  
  #Display info. about interfaces of the router
  def show_interface(dpid)
  	logger.info "Port\t\tMac address\t\tIP address\t\tNetmask length"
  	Interface.each do |each|
  		logger.info "#{each.port_number}\t\t#{each.mac_address}\t#{each.ip_address}\t\t#{each.netmask_length}"
  	end
  end
